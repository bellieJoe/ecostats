...
const formName = FormEnum.BIODIVERSITY_16;
const sector = Sector.BIODIVERSITY;

const Biodiversity_16_Dashboard = () => {
        ...
        if(data[0]){
            setChartData(_.uniqBy(data, "municipality").map(a => {
                return {
                    ...
                    "Number of Permits Issued" : _.sum(data.filter(b => b.municipality == a.municipality).map(c => c.number_of_permits_issued)),
                    "Revenue Generated (Php)" : _.sum(data.filter(b => b.municipality == a.municipality).map(c => c.revenue_generated))
                }
            }))
...
export default Biodiversity_16_Dashboard;